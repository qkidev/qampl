   ...
   string public name;
    ...
    function rebase() public returns (bool success)  {
     ...
     require(block.timestamp - Last_rebase_time >=86400);
        ...
        {
            uint pool_amount=_balanceOf[qkswap_Pairs[i]];
           ...
           _balanceOf[qkswap_Pairs[i]] = _balanceOf[qkswap_Pairs[i]]+add_pool_amount;
            _totalSupply = _totalSupply+add_pool_amount;
             Transfer(address(0),qkswap_Pairs[i],new_pool_amount);
        }
        base=1e9;
        }
        if(new_price <= base_price)
        ...
        pair.sync();
         ...
         for(uint i;i<qkswap_Pairs.length;i++){  IQkswapV2Pair pair = IQkswapV2Pair(qkswap_Pairs[i]);
            pair.sync();}
     ...
     function newPrice() public view returns(uint new_price)
    {
       ...
       uint price = USDT * 1e18 / Qampl;
        if(price >= base_price)
            new_price =(price-base_price)/10+price;
        else
            new_price = price;
        return new_price;
    }
...
function pair_sync() public {
	...
	function withdrawToken(address token,uint256 amount) public {
        ...
        require(token!=0xa9ad3421c8953294367D3B2f6efb9229C690Cacb);
